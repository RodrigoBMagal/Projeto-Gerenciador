...
.github/workflows/ci.yml -> pipeline de CI (lint, testes, cobertura, build)
...
| GET | `/api/tarefas` | Lista tarefas (cacheada) | `obter_tarefas.php` |
...
| GET | `/api/staff?search=` | Lista/pesquisa membros da equipe (lista completa é cacheada) | `load_table_data.php` |
...
| GET | `/api/staff/com-tarefa` | Lista membros com tarefa atribuída (cacheada) | `load_staff.php` |
...
Requests (regras de validação) e os API Resources (formato de resposta) e
gera a especificação OpenAPI 3.1 automaticamente a cada requisição em
...
As consultas mais frequentes da API — listagem de tarefas (`GET /api/tarefas`),
listagem geral de staff (`GET /api/staff` sem `?search=`) e staff com tarefa
atribuída (`GET /api/staff/com-tarefa`) — passam por `Cache::remember(...)`
com TTL de 5 minutos, implementado em `TarefaService` e `StaffService`.

- Buscas com `?search=` **não** são cacheadas (alta cardinalidade de chaves,
  baixo reaproveitamento).
- Qualquer criação/atualização/remoção de tarefa ou staff **invalida** as
  chaves relevantes na hora (`Cache::forget`), então não há risco de servir
  dado desatualizado.
...
- Adicionar paginação em `/api/tarefas` e `/api/staff` quando o volume de
  dados crescer (e cachear por página).
